...
FERA2011_aus = [1, 2, 6, 7, 10, 12, 15, 17, 25, 26];
...
for a=1:numel(all_dataset_aus)
    ...
    if(~isempty(find(BP4D_aus == au, 1)))
        ...
        [train_samples_bp4d, train_labels_bp4d, valid_samples_bp4d, valid_labels_bp4d, ~, PC, means, scaling] = Prepare_HOG_AU_data_generic(train_recs, devel_recs, au, BP4D_dir, hog_data_dir, pca_loc);
    ...
    if(~isempty(find(FERA2011_aus == au, 1)))
        ...
        shared_defs;
        % load the training and testing data for the current fold
        [train_samples_f2011, train_labels_f2011, valid_samples_f2011, valid_labels_f2011, ~, PC, means, scaling] = Prepare_HOG_AU_data_generic(train_recs, devel_recs, au, rest_aus, FERA2011_dir, hog_data_dir, pca_loc);
    ...
    if(~isempty(find(SEMAINE_aus == au, 1)))
        ...
        shared_defs;
        % load the training and testing data for the current fold
        [train_samples_sem, train_labels_sem, valid_samples_sem, valid_labels_sem, ~, PC, means, scaling] = Prepare_HOG_AU_data_generic(train_recs, devel_recs, au, rest_aus, SEMAINE_dir, hog_data_dir, pca_loc);
    ...
    if(~isempty(find(DISFA_aus == au, 1)))
        ...
        [train_samples_disf, train_labels_disf, valid_samples_disf, valid_labels_disf, ~, PC, means, scaling, valid_ids, valid_success] = Prepare_HOG_AU_data_generic(users_train, users_valid_disfa, au, rest_aus, hog_data_dir);
    ...
    if(~isempty(find(UNBC_aus == au, 1)))
        ...
        % load the training and testing data for the current fold
        [train_samples_unbc, train_labels_unbc, valid_samples_unbc, valid_labels_unbc, ~, PC, means, scaling] = Prepare_HOG_AU_data(users_train, users_valid_unbc, au, rest_aus, UNBC_dir, hog_data_dir, pca_loc);
    ...
    name = sprintf('mat_models/AU_%d_static.mat', au);
    ...
    save(name, 'model', 'F1s', 'predictions_all', 'valid_labels', 'eval_ids', 'dataset_ids');
       
    % Write out the model
    name = sprintf('classifiers/AU_%d_static.dat', au);
