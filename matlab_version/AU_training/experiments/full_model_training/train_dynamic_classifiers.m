...
UNBC_aus = [6, 7, 9, 10, 12, 25, 26];
DISFA_aus = [1, 2, 4, 5, 6, 9, 12, 15, 17, 20, 25, 26];

% For dynamic models we skip FERA as that does not work as well due to the
% videos being very short and not containing as much dynamics (no neutral
% expression in lots of them)
...
for a=1:numel(all_dataset_aus)
    ...
    if(~isempty(find(SEMAINE_aus == au, 1)))
        ...
        shared_defs;
        % load the training and testing data for the current fold
        [train_samples_sem, train_labels_sem, valid_samples_sem, valid_labels_sem, ~, PC, means, scaling] = Prepare_HOG_AU_data_generic_dynamic(train_recs, devel_recs, au, rest_aus, SEMAINE_dir, hog_data_dir, pca_loc);
    ...
    if(~isempty(find(DISFA_aus == au, 1)))
        ...
        [train_samples_disf, train_labels_disf, valid_samples_disf, valid_labels_disf, ~, PC, means, scaling, valid_ids, valid_success] = Prepare_HOG_AU_data_generic_dynamic(users_train, users_valid_disfa, au, rest_aus, hog_data_dir);
    ...
    if(~isempty(find(UNBC_aus == au, 1)))
        ...
        % load the training and testing data for the current fold
        [train_samples_unbc, train_labels_unbc, valid_samples_unbc, valid_labels_unbc, ~, PC, means, scaling] = Prepare_HOG_AU_data_dynamic(users_train, users_valid_unbc, au, rest_aus, UNBC_dir, hog_data_dir, pca_loc);
